...
这里要特别注意，“K8s 完成服务注册”并不是应用自己主动去某个注册中心调用接口，而是平台控制面自动做了下面这条链路：

1. Pod 启动并拿到 Pod IP
2. Service 用 selector 匹配出属于自己的 Pod
3. EndpointSlice Controller 判断这些 Pod 是否 Ready
4. 只有 Ready 的 Pod，才会被加入对应的 EndpointSlice/Endpoints
5. CoreDNS 和 kube-proxy 监听这些变化，更新 DNS 解析结果和转发规则

所以更准确地说：

**K8s 里的“注册”是控制面自动把可用 Pod 纳入服务端点列表，而不是应用实例自己上报地址。**
...
如果继续追问“那调用方到底发现了什么”，答案要分两种：

- 普通 Service：调用方发现的是一个稳定的 `ClusterIP`
- Headless Service：调用方发现的是多个真实 Pod IP

第二种方式更像传统 RPC 注册中心，因为客户端能直接拿到实例列表并自己做负载均衡。

#### Headless Service 为什么像传统注册中心
...
DNS 返回的就不再是一个虚拟 IP，而是多个 Pod 地址。

这样一来：

- 调用方能直接看到实例列表
- gRPC 或自定义 RPC 客户端可以自己做轮询、权重、连接池管理
- 因为没有 `ClusterIP`，客户端通常会直接连接具体 Pod
- 这和“从注册中心拿地址列表，再本地选节点”就很接近了
...
从服务发现角度看，`readinessProbe` 其实是在回答一个非常关键的问题：

**这个 Pod 现在要不要继续出现在服务地址列表里。**

比如下面这些场景里，服务可能还活着，但不应该再接流量：
...
这也是为什么 K8s 的服务发现，经常会比“单纯发心跳”更贴近真实可用性。
...
如果面试官问你“K8s 是怎么完成服务注册与发现的”，一个够完整的回答可以是：

“K8s 里服务实例是 Pod。Service 通过 label selector 选出属于自己的 Pod，控制面再结合 readiness 状态把 Ready 的 Pod 写入 EndpointSlice。CoreDNS 负责把服务名解析成 ClusterIP 或者 Headless Service 下的一组 Pod IP，kube-proxy / IPVS 再把请求转发到具体实例上。所以 K8s 的服务注册与发现本质上是平台自动维护可用实例列表，而不是应用自己主动往注册中心注册地址。”
...
- 在 K8s 环境中，Service、EndpointSlice、CoreDNS、kube-proxy 和探针常常已经构成了一套很强的原生服务发现体系。
- 普通 Service 适合稳定入口，Headless Service 更接近传统 RPC 那种“拿到实例列表再自己选节点”的模式。
