...
生产排障层和面试追问层。它不重复介绍 Sentinel 是什么，而是专门拆下线判定、选主、故障转移和脑裂风险。
...
## 实战落地：Sentinel 部署要避免“看起来有多数派”

Sentinel 最怕部署形态不认真。常见建议是至少 3 个 Sentinel，并且分散在不同机器或可用区；更重要的是，不要把所有 Sentinel 和 Redis 节点放在同一台宿主机上，否则机器故障时多数派也一起消失。

上线前需要确认：

- `quorum` 和 Sentinel 数量匹配，不能过低导致误切，也不能过高导致切不动。
- 应用客户端配置的是 Sentinel 地址列表，而不是单个 Sentinel。
- Sentinel、master、replica 的网络路径稳定，跨机房部署要特别谨慎。
- 故障转移脚本或通知通道可用，切换事件能进告警系统。

Sentinel 的核心价值不是“有一个监控进程”，而是让故障判断、选主、切换、通知应用形成自动闭环。

## 生产问题处理：脑裂和误切怎么处置

如果线上出现疑似脑裂，处理顺序要保守：

1. 先确认当前对外写入的 master 是哪一个，必要时临时阻断旧 master 写流量。
2. 保留旧 master 和新 master 的关键 key、offset、日志，不要马上重启清空。
3. 检查 `min-replicas-to-write` 是否生效，确认旧 master 是否在隔离期间接受过写入。
4. 对业务敏感数据做差异核验，不能只靠 Redis 自动同步覆盖。
5. 复盘 Sentinel 日志，定位是网络分区、机器卡顿、Sentinel 数量不足，还是阈值配置不合理。

误切不是单纯 Redis 配置问题，通常还牵涉网络、部署拓扑、客户端重连和业务幂等。把这些一起复盘，才不会下次继续在同一个点跌倒。
