...
场景实战层。它把第 2 课的数据类型落到对象缓存和购物车建模，重点解决 String JSON 和 Hash 该怎么选。
...
## 实战落地：购物车不是一个 Hash 就结束

购物车用 Redis 做缓存时，常见建模是 `cart:{userId}` 对应一个 Hash，field 是 `skuId`，value 是数量、勾选状态或精简 JSON。但生产里还要继续拆问题：

- 是否按店铺、业务线或租户拆 key，避免一个用户购物车过大。
- 商品价格、库存、上下架状态是否只放引用，还是直接冗余快照。
- 登录前匿名购物车如何和登录后购物车合并，合并是否幂等。
- DB 是最终存储还是 Redis 是临时态，提交订单后谁负责清理。
- TTL 如何设置，长期不活跃用户的购物车是否自动回收。

如果购物车要支持复杂筛选、历史追踪、营销规则，Redis 只适合做热路径缓存，最终状态仍然应该落数据库。Redis 负责快，数据库负责可审计和可恢复。

## 生产问题处理：对象缓存常见故障

对象缓存出问题时，通常表现为字段丢失、旧值覆盖新值、单个 key 过大或缓存和 DB 不一致。处理时可以这样拆：

- 字段丢失：检查是否有全量覆盖写，Hash 局部更新是否和 JSON 全量更新混用。
- 旧值覆盖：检查回源后写缓存是否带版本号，是否发生慢请求后写入旧数据。
- 单 key 过大：用 `HLEN`、`MEMORY USAGE` 定位，按业务维度拆 key。
- 不一致：先删缓存触发回源，再查写路径是否缺少删除缓存或补偿重试。

如果是购物车这类用户体验敏感但可恢复的数据，事故处理可以先保障用户可继续下单，再异步修复展示差异；不要为了追求缓存完全正确把核心下单链路卡死。
