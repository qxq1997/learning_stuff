...
原理深挖层。它负责澄清“Redis 单线程”这句口号，帮助你解释主线程阻塞、I/O 多线程和后台任务的真实边界。
...
理解 Redis 线程模型的实际价值，是知道哪些操作会挤占主线程：

- 大 key 读写会让命令执行时间变长。
- 大对象同步删除会阻塞主线程，应优先考虑 `UNLINK`。
- 慢 Lua、全量扫描、大范围集合运算都会破坏事件循环节奏。
- 持久化 fork、AOF rewrite、异步删除虽然有后台线程参与，但仍可能带来资源抖动。
- Redis 6 的 I/O 多线程主要缓解网络读写，不会让重命令变轻。

所以生产优化不要一听“多线程”就以为可以随便跑重操作。命令执行主线程仍然是 Redis 的核心瓶颈点。

## 生产问题处理：主线程阻塞怎么定位

怀疑 Redis 主线程阻塞时，可以组合这些信号：

- `SLOWLOG` 有明显慢命令或慢 Lua。
- `INFO stats` 中 `instantaneous_ops_per_sec` 降低，但客户端等待增加。
- `INFO clients` 中 blocked clients 增多。
- 延迟监控或 `LATENCY DOCTOR` 出现 fork、command、expire-cycle 等事件。
- 业务日志显示大量 Redis 超时集中在某个时间段。

处理动作通常是先止血：暂停批处理、禁用危险命令、拆分大 key、降低迁移或重写压力。之后再做结构治理，别只把超时时间调大，那只是让故障更晚暴露。
