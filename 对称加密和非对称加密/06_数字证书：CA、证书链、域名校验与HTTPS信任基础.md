...
它们在大多数工程语境里，**通常说的是同一类东西：TLS 握手时服务端出示的那张数字证书。**
...
**TLS certificate，本质上就是 TLS 连接里用来证明“我是我”的证书。**
...
- 公钥：像门锁
- TLS certificate：像“带公章的门锁说明书 + 身份证明”

客户端不是只看“你给了我一把公钥”，而是要看：
...
#### 9.1 TLS certificate 不是“一个单独的加密算法”

它不是 AES、RSA、ECC 这种算法本身。  
它更像是一份承载身份、公钥和签名信息的数据结构。

#### 9.2 TLS certificate 也不只是“一个公钥文件”

证书里确实有公钥，但它比单纯公钥多了很多关键上下文：
...
**它就是 TLS 握手时出示的数字证书，用来证明服务端身份，并把服务端公钥可信地交给客户端。**
...
#### 11.3 误区三：我有证书，就说明我一定安全

证书只是信任链的一部分。  
如果私钥泄露，攻击者依然可能冒充你。  
所以证书和私钥必须一起看。

#### 11.4 误区四：证书和数字签名是两个完全无关的概念

其实不是。  
证书本身就是靠签名建立信任的。  
你前一课学的数字签名，正好在这里变成“信任体系的基石”。

### 12. 作为后端工程师，你应该有的稳定认知
